...
describe('checkin.service.js', () => {
  ...
  describe('checkInRegistration', () => {
    it('should checkin successfully', async () => {
      regRepo.getRegistrationById.mockResolvedValue({ status: 'confirmed', workshop_id: 1 });
      ...
      const result = await checkInRegistration(1, new Date().toISOString(), { role: 'staff', id: 2 });
    ...
    it('should throw if already checked in', async () => {
      regRepo.getRegistrationById.mockResolvedValue({ status: 'confirmed' });
      checkinRepo.findCheckinByRegistrationId.mockResolvedValue({ id: 1 });

      await expect(checkInRegistration(1, null, null)).rejects.toThrow('Dang ky da duoc check-in');
      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
    });

    it('should throw if registration not confirmed', async () => {
      regRepo.getRegistrationById.mockResolvedValue({ status: 'cancelled' });

      await expect(checkInRegistration(1, null, null)).rejects.toThrow('Dang ky chua duoc xac nhan hoac da huy');
      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
    });
  });
});
